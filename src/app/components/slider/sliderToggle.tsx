  ...
  return (
    <div className={`flex flex-row items-center gap-4 mt-1 ${className}`}>
      ...
      <input
        ...
        disabled={!enabled}
        className="w-full h-2 bg-gray-1 rounded-lg appearance-none disabled:opacity-50 
                cursor-pointer accent-navy-text"
      />
      {/* 현재 값 표시 */}
      <span className="text-sm1 w-4">{value.toFixed(1)}</span>
    </div>
  );
};
